...
import chenfei.com.adapter.GerenLinelistadapter;
 ...
 */
public class GerenFragment extends Fragment implements BaseQuickAdapter.RequestLoadMoreListener, SwipeRefreshLayout.OnRefreshListener {
    private View gerenyuyuefragment;
    private RecyclerView mRecyclerView;
    private GerenLinelistadapter madpter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (gerenyuyuefragment == null) {
            gerenyuyuefragment = inflater.inflate(R.layout.fragment_line, container,
                    false);
        }
        mRecyclerView = (RecyclerView) gerenyuyuefragment.findViewById(R.id.line_list);
        mSwipeRefreshLayout = (SwipeRefreshLayout) gerenyuyuefragment.findViewById(R.id.swipeLayout);
        mSwipeRefreshLayout.setOnRefreshListener(this);
        mRecyclerView.setLayoutManager(new LinearLayoutManager(getActivity()));
        super.onCreateView(inflater, container, savedInstanceState);
        gerenyuyuefragment.forceLayout();

        //缓存的rootView需要判断是否已经被加过parent， 如果有parent需要从parent删除，要不然会发生这个rootview已经有parent的错误。
        ViewGroup parent = (ViewGroup) gerenyuyuefragment.getParent();
        if (parent != null) {
            parent.removeView(gerenyuyuefragment);
        }
        return gerenyuyuefragment;
    }
    ...
    private void initAdapter() {
        madpter = new GerenLinelistadapter(getActivity(), R.layout.guanfanglineitem, mData, PAGE_SIZE);
